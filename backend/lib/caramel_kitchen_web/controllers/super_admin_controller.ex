defmodule CaramelKitchenWeb.SuperAdminController do
  ...
  def list_users(conn, params) do
    ...
    json(conn, %{data: Enum.map(users, fn u ->
      %{id: u.id, email: u.email, name: u.name, role: u.role,
        subscription_tier: u.subscription_tier, inserted_at: u.inserted_at,
        last_sign_in_at: u.last_sign_in_at, sign_in_count: u.sign_in_count,
    ...
    try do
      case Redix.command(:redix, ["INFO", "stats"]) do
        {:ok, info} ->
          ...
          |> Map.take(["keyspace_hits", "keyspace_misses", "connected_clients", "used_memory_human"])
        _ -> %{}
      end
    rescue
      _ -> %{error: "unavailable"}
