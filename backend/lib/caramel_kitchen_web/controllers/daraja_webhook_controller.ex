  ...
  def stk_callback(conn, params) do
    ...
    with :ok <-
           Daraja.Callback.Security.verify(
             ...
             # bypass if not provided in dev
             provided_secret: params["token"] || shared_secret
           ),
  ...
  defp handle_payment_result(%Daraja.Express.Callback.Result{} = callback) do
    if callback.result_code == 0 do
      ...
      sub =
        Repo.one(
          from s in Subscription,
            where:
              s.mpesa_receipt_number == ^callback.checkout_request_id and s.status == "pending"
        )
      ...
      if sub do
        ...
        Logger.warning(
          "Received successful STK push callback but no pending subscription found for #{callback.checkout_request_id}"
        )
      end
    else
