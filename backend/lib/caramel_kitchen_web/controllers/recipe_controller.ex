defmodule CaramelKitchenWeb.RecipeController do
  ...
  def search(conn, %{"q" => q} = params) do
    ...
    json(conn, %{
      data: Enum.map(results, fn %{recipe: r, rank: rank} ->
...
defmodule CaramelKitchenWeb.FeedController do
  ...
  def personalised(conn, params) do
    ...
    json(conn, %{
      data: %{
        for_you:  Enum.map(results, fn %{recipe: r, taste_score: ts} ->
          %{id: r.id, title: r.title, thumbnail_url: r.thumbnail_url,
            taste_score: ts, taste_tags: r.taste_tags, total_time_mins: r.total_time_mins,
            calories: r.calories, dietary_flags: r.dietary_flags}
        end),
        trending: Enum.map(trending, fn r ->
          %{id: r.id, title: r.title, thumbnail_url: r.thumbnail_url,
