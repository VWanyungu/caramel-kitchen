defmodule CaramelKitchen.Monetisation do
  ...
  def handle_webhook(%{"type" => event_type, "data" => %{"object" => obj}}) do
    ...
    case event_type do
      "checkout.session.completed"      -> handle_checkout_completed(obj)
      ...
      "customer.subscription.updated"   -> handle_subscription_updated(obj)
      _other ->
  ...
  defp handle_checkout_completed(%{"client_reference_id" => user_id, "customer" => customer_id,
                                    "subscription" => stripe_sub_id, "metadata" => meta}) do
  ...
  defp handle_payment_succeeded(%{"subscription" => stripe_sub_id,
                                   "lines" => %{"data" => [%{"period" => period} | _]}}) do
  ...
  defp handle_subscription_deleted(%{"id" => stripe_sub_id}) do
    with {:ok, sub} <- find_by_stripe_sub(stripe_sub_id),
      ...
      sub
      |> Ecto.Changeset.change(%{status: "canceled", canceled_at: DateTime.utc_now() |> DateTime.truncate(:second)})
      |> Repo.update()
  ...
  defp upsert_subscription(attrs) do
    case Repo.one(from s in Subscription, where: s.user_id == ^attrs.user_id) do
      nil ->
        %Subscription{}
        |> Ecto.Changeset.cast(attrs, [:user_id, :stripe_customer_id, :stripe_sub_id, :plan, :status])
        |> Repo.insert()
  ...
  defp get_price_id("premium"),    do: Application.fetch_env!(:caramel_kitchen, :stripe_premium_price_id)
  defp get_price_id("creator_pro"), do: Application.fetch_env!(:caramel_kitchen, :stripe_creator_price_id)
